...
begin
  ...
  while (Length(srv) > 2) and (rlei <= 255) do begin
    ...
    Delete(srv, 1, Pos('\', srv));
    if key = 'hostname' then info.ServerName := info.RuleData[rlei].Value;
    if key = 'mapname' then info.Map := info.RuleData[rlei].Value;
    if key = 'game' then info.GameMod := info.GameMod + '/' + info.RuleData[rlei].Value;
    if key = 'maxclients' then info.MaxPlayers := StrToInt(info.RuleData[rlei].Value);
    if key = 'needpass' then s := info.RuleData[rlei].Value;
    Inc(rlei);
  end;
