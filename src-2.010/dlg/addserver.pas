...
type
  ...
  private
    function AddToList(ipPort, ip, port: String): Boolean;
  ...
  for p := 0 to mmoServers.Lines.Count-1 do begin
    ...
    if chkList.Checked then begin //its single ip/dns'
      if Pos(':', s) > 0 then begin
        ...
        addy := Trim(s);
        with frmMain do
          port := gameData[activeGame].DefaultPort;
        ipPort := addy + ':' + port;
      end;
      ip := ResolveIP(addy);
      if ip <> '' then begin
        if AddToList(ipPort, ip, port) then Inc(addnum);
      end
      else
      ...
      while Length(s) > 1 do begin
        if Ord(s[1]) in [48..57] then begin
          ...
          if not b and (m = 3) then begin
            ...
            if AddToList(ipPort, addy, port) then Inc(addnum);
          end;
        end;
        Delete(s, 1, 1);
      end;
...
function TfrmAddServer.AddToList(ipPort, ip, port: String): Boolean;
var
  i: Integer;
  b: Boolean;
begin
  with frmMain do begin
    ...
    b := False;
    repeat //dup
      if (frmMain.serverData[i].ServerName <> '') then
    ...
    if not b then begin
      ...
      if (i <= 1023) and b then begin
        ...
        with frmMain do
          serverData[i].GameMod := gameData[activeGame].GameName;
        serverData[i].IP := ip;
