...
uses
  ...
  ShellAPI, Clipbrd, Winsock, Masks, TB2Item, TB2Dock, TB2Toolbar;
...
type
  TDLLServerData = record
    ...
    GameMod: PChar;
    IconIndex: Integer;
    PlayerData: Array[0..127] of TDLLPlayerData;
    RuleData: Array[0..255] of TDLLRuleData;
  end;
...
type
  TGet = function (var server: TThisServer; var info: TDLLServerData): boolean; stdcall;
...
type
  TfrmMain = class(TForm)
    ...
    mnuRule: TPopupMenu;
    mniCopyRule: TMenuItem;
    mniPluginInfo: TTBItem;
    ...
    procedure lvwRuleContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
    procedure mniCopyRuleClick(Sender: TObject);
    procedure mniPluginInfoClick(Sender: TObject);
  public
    gameData: Array[0..63] of TGameData;
...
const
  FIZZ_VERSION: String = '2.01'; //Version changes: Project->Options...->Version Info and fizz.txt

implementation

uses Server, Disk, AddServer, GameSetup, Options, Splash, About, Join, Refresh, Filter, DLLInfo;
...
begin
  Result := 0;
  s1 := frmMain.GameData[frmMain.activeGame].ID;
  if (Data = frmMain.playerColOrder[0]+1) and ((s1 = 'QW') or (s1 = 'VQ')) then begin //QW
...
begin
  lvwPlayer.Items.BeginUpdate;
  lvwPlayer.Items.Clear;
  x := StrToInt(GetServerItem(index, 6));
  i := 0;
  while (serverData[x].PlayerData[i].Name <> '') and (i <= 127) do begin
    lvwPlayer.Items.Add;
    for j := 0 to lvwPlayer.Columns.Count+2 do //-1+2
      lvwPlayer.Items[i].SubItems.Add('');
    SetPlayerItem(i, 0, serverData[x].PlayerData[i].Name);
    SetPlayerItem(i, 1, IntToStr(serverData[x].PlayerData[i].Frags));
    SetPlayerItem(i, 2, serverData[x].PlayerData[i].Skin);
    SetPlayerItem(i, 3, IntToStr(serverData[x].PlayerData[i].Ping));
    SetPlayerItem(i, 4, IntToStr(serverData[x].PlayerData[i].Connect));
    SetPlayerItem(i, 5, IntToStr(serverData[x].PlayerData[i].UserID));
    if (serverData[x].PlayerData[i].TopColour <> -1) and
       (serverData[x].PlayerData[i].BottomColour <> -1) then begin
      SetPlayerItem(i, 6, IntToStr(serverData[x].PlayerData[i].TopColour));
      SetPlayerItem(i, 7, IntToStr(serverData[x].PlayerData[i].BottomColour));
    end
    else begin
      SetPlayerItem(i, 6, '-1');
      SetPlayerItem(i, 7, '-1');
  ...
  lvwRule.Items.Clear;
  i := 0;
  while (serverData[x].RuleData[i].Name <> '') and (i <= 225) do begin
    lvwRule.Items.Add;
    lvwRule.Items[i].Caption := serverData[x].RuleData[i].Name;
    lvwRule.Items[i].SubItems.Add(serverData[x].RuleData[i].Value);
    Inc(i);
  end;
  lvwRule.Items.EndUpdate;
end;
...
begin
  for i := 0 to lvwServer.Items.Count-1 do //refresh icon
    lvwServer.Items[i].ImageIndex := 0;
  with RefreshList.Create(True) do begin
    ...
    qrFunc := gameData[activeGame].GetData;
    qrGame := activeGame;
...
begin
  ...
  with RefreshList.Create(True) do begin
    FreeOnTerminate := True;
    while item <> nil do begin
      lvwServer.Items[item.Index].ImageIndex := 0; //icon
...
begin
  ...
  with frmGameSetup do begin
    ...
    while (gameData[i].GameName <> '') and (i <= 63) do begin
      cbxGame.Items.Add(gameData[i].GameName);
      with tmpGameData[i] do begin
    ...
    txtCmdLine1.Text := tmpGameData[0].CommandLine1;
    txtCmdLine2.Text := tmpGameData[0].CommandLine2;
    txtParam1.Text := tmpGameData[0].Param1;
    txtParam2.Text := tmpGameData[0].Param2;
    chkFile.Checked := tmpGameData[0].CreateTextFile;
    txtFileName.Text := tmpGameData[0].TextFileName;
    mmoFileContents.Text := tmpGameData[0].TextFileContents;
    chkInstalled.Checked := tmpGameData[0].Installed;
    Last := 0;
    cbxGame.ItemIndex := 0;
  end;
  if frmGameSetup.ShowModal = mrOK then begin
    activeGame := -1;
    SetGameBox; //activegame gets set here
    IsGame;
    UpdateList(True);
  end;
...
begin
  ...
  while (gameData[i].GameName <> '') and (i <= 63) do begin
    if gameData[i].Installed then begin
      ...
      cboGame.Items.Add(gameData[i].GameName);
      b := True;
    end;
    Inc(i);
  end;
...
//  lvwServer.SmallImages := gameData[activeGame].
 // gameData[activeGame].Icon
  ...
  for i := 0 to 1023 do begin
    if FilterItem(serverData[i], filterServer) then begin
      ...
      if serverData[i].MaxPlayers <> 0 then
        SetServerItem(j, 4, IntToStr(serverData[i].Players) + '/' + IntToStr(serverData[i].MaxPlayers));
      SetServerItem(j, 5, serverData[i].GameMod);
      SetServerItem(j, 6, IntToStr(i)); //tag
      lvwServer.Items[j].ImageIndex := serverData[i].IconIndex;
      Inc(j);
    end;
  end;
...
begin
  ...
  i := 0;
  while (gameData[i].GameName <> '') and (i <= 63) do begin
    if cboGame.Text = gameData[i].GameName then begin
      activeGame := i;
      Break;
    end;
    Inc(i);
    ...
    for i := 0 to 7 do begin
      ...
      if filter[i].Used then begin
        ...
        case filter[i].TypeKey of
          0: begin
               j := 0;
               while (s.RuleData[j].Name <> '') and (j <= 255) do begin
                 if s.RuleData[j].Name = a[1] then begin
                   t := s.RuleData[j].Value;
                   Break;
                 end;
                 Inc(j);
               end;
          ...
          1: case StrToInt(filter[i].Key) of
               ...
               1: t := s.Ping;
               2: t := s.Map;
               3: t := IntToStr(s.Players); //players on server
               4: t := Copy(s.GameMod, Pos('/', s.GameMod)+1, Length(s.GameMod)); //mod
               5: if (s.PlayerData[0].Name = '') or (s.RuleData[0].Name = '') then
                    t := '0' //didnt respond
                  else
                    t := '1'; //responded
             end;
...
begin
  ...
  with Sender as TListView do begin
    x := StrToInt(Item.SubItems[7]);
    y := StrToInt(Item.SubItems[6]);
    if (x <> -1) and (y <> -1) and (playerColOrder[0] = -1) then begin
    //  if (GameData[activeGame].ID = 'QW') then begin
        SmallImages.Draw(Canvas, r.Left+3, r.Top, x); //first image
        SmallImages.Draw(Canvas, r.Left+21, r.Top, y); //second image
        Canvas.TextOut(r.Left+39, r.Top+2, Item.Caption);
    //  end
    //  else
   //     Canvas.TextOut(r.Left+1, r.Top+2, Item.Caption);
    end
    else
...
begin
  ...
  with Sender as TListView do begin
    if SubItem < Columns.Count-1 then begin
      ...
      x := StrToInt(Item.SubItems[7]);
      y := StrToInt(Item.SubItems[6]);
      if (x <> -1) and (y <> -1) and (playerColOrder[0] > -1) and (SubItem = playerColOrder[0]) then begin //qw
...
begin
  i := 0;
  j := -1;
  while (GameData[i].GameName <> '') and (i <= 64) do begin
    if cboGame.Items[Index] = GameData[i].GameName then begin
      j := i;
      Break;
    end;
    Inc(i);
  ...
  if j > -1 then
    GameData[i].Icons.Draw(cboGame.Canvas, Rect.Left - 2, Rect.Top, 2);
    //DrawIcon(cboGame.Canvas.Handle, 10, 10, GameData[i].Icon);
  //imgServer.Draw(cboGame.Canvas, Rect.Left, Rect.Top, icon);
  cboGame.Canvas.TextOut(Rect.Left + 16 + 8, Rect.Top + 1, cboGame.Items[Index]);
end;
...
begin
  ...
  s[4] := IntToStr(serverData[i].Players) + '/' + IntToStr(serverData[i].MaxPlayers);
//  s[5] := serverData[i].GameMod;
  Clipboard.AsText := s[0] + '|' +
                      s[2] + 'ms|' +
                      s[1] + '|' +
                      s[3] + '|' +
                      s[4];
...
begin
  if activeGame > -1 then begin
    ...
    lvwServer.SmallImages := gameData[activeGame].Icons;
  end
  else begin
...
procedure TfrmMain.lvwRuleContextPopup(Sender: TObject; MousePos: TPoint; var Handled: Boolean);
begin
  mniCopyRule.Enabled := not ((lvwRule.Items.Count = 0) or not (lvwRule.SelCount >= 1));
end;

procedure TfrmMain.mniCopyRuleClick(Sender: TObject);
begin
  Clipboard.AsText := lvwRule.Items[lvwRule.Selected.Index].Caption + ': ' +
                      lvwRule.Items[lvwRule.Selected.Index].SubItems[0];
end;

procedure TfrmMain.mniPluginInfoClick(Sender: TObject);
var
  i, j: Integer;
begin
  ...
  for i := 0 to 63 do begin
    if (GameData[i].ID <> '') then begin
      with frmDLLInfo do begin
        lvwList.Items.Add;
        lvwList.Items[j].Caption := GameData[i].GameName;
        lvwList.Items[j].SubItems.Add(GameData[i].Author);
        lvwList.Items[j].SubItems.Add(GameData[i].Version);
        lvwList.Items[j].SubItems.Add(GameData[i].ID);
        Inc(j);
      end;
    end;
  end;
  frmDLLInfo.ShowModal;
  frmDLLInfo.Release;
end;
